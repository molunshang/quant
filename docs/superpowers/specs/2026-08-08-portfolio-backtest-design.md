...
- **按时间点（time step）推进**：本期统一时间轴 = 全部标日的并集（`daily`）；每个时间点设置 `ctx.time`、按需切片暴露各标的历史 → 调用 `handle_data(ctx)`（策略内 `ctx.sell`/`ctx.buy` **立即按当前 bar 收盘价逐单撮合**，先执行先成交；各标按自身类型应用 A股规则）→ 记录组合净值。引擎的时间推进抽象为「下一个时间点」，与 bar 频率解耦，为分钟级预留
...
> `ctx.sell` / `ctx.buy` 在调用时**立即按当前 bar 收盘价逐单撮合**（同一时间点内先执行的订单先成交）。`pct` 相对调用时刻的净值计算，因此**先卖后买**时卖出所得现金才可用于买入；策略若依赖卖出腾出的资金，须先调 `sell` 再 `buy`。引擎按 A股规则撮合，`buy`/`sell` 返回 `bool` 表示是否成交，策略不应假设下单必成。
