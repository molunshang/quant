...
symbol | str | 标的代码  
volume | int | 数量\(指股数\)  
side | int | 参见[订单委托方向](枚举常量.md#orderside委托方向)  
order\_type | int | 参见[订单委托类型](枚举常量.md#ordertype委托类型)  
position\_effect | int | 参见[开平仓类型](枚举常量.md#positioneffect开平仓类型)  
price | float | 价格（限价委托的委托价格；市价委托的保护价，沪市实盘市价单必填字段，股票需要保留两位小数，回测仿真可不填写，参考下文注意点6。）  
trigger\_type | int | 参见[期货条件单触发方式](枚举常量.md#ordertriggertype期货ctp条件单触发方式)，非条件单为0  
...
symbol | str | 标的代码  
volume | int | 期望的最终数量  
position\_side | int | 表示将多仓还是空仓调到目标持仓量，参见 [持仓方向](枚举常量.md#positionside持仓方向)  
order\_type | int | 参见[订单委托类型](枚举常量.md#ordertype委托类型)  
price | float | 价格（限价委托的委托价格；市价委托的保护价，沪市实盘市价单必填字段，股票需要保留两位小数，回测仿真可不填写，参考下文注意点5。）  
account | account id or account name or None | 帐户  
...
参数名 | 类型 | 说明  
---|---|---  
orders | list\[order\] | 委托对象列表，其中委托至少包含交易接口的必选参数，参见[order 对象](数据结构.md#order-委托对象)  
...
参数名 | 类型 | 说明  
---|---|---  
wait\_cancel\_orders | list\[dict\] | 传入单个字典. 或者 list 字典. 每个字典包含 key: cl\_ord\_id, account\_id， 参见[order 对象](数据结构.md#order-委托对象)  
...
类型 | 说明  
---|---  
list\[execrpt\] | 回报对象列表， 参见[ExecRpt 回报对象](数据结构.md#execrpt-回报对象)  
