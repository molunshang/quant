...
参数名 | 类型 | 中文名称 | 必填 | 默认值 | 参数用法说明  
---|---|---|---|---|---  
industry\_code | str | 行业代码 | Y | 无 | 需要查询成分股的行业代码，可通过[stk\_get\_industry\_category](API介绍/股票增值数据函数（付费）.md#stk-get-industry-category-查询行业分类)获取, 查看[行业分类](</docs2/docs/index.html#行业分类)  
...
参数名 | 类型 | 中文名称 | 必填 | 默认值 | 参数用法说明  
---|---|---|---|---|---  
symbols | str or list | 股票代码 | Y | 无 | 多个代码可用 ，使用时参考[symbol](变量约定.md#symbol-代码标识) 采用 str 格式时，多个标的代码必须用英文逗号分割如：`'SHSE.600008,SZSE.000002'` 采用 list 格式时，多个标的代码示例：`['SHSE.600008', 'SZSE.000002']`  
source | str | 行业来源 | N | 'zjh2012' | 'zjh2012'- 证监会行业分类 2012（默认）， 'sw2021'- 申万行业分类 2021, 查看[行业分类](</docs2/docs/index.html#行业分类)  
level | int | 行业分级 | N | 1 | 1 - 一级行业（默认），2 - 二级行业，3 - 三级行业  
...
参数名 | 类型 | 中文名称 | 必填 | 默认值 | 参数用法说明  
---|---|---|---|---|---  
sector\_type | str | 板块类型 | Y | 无 | 只能选择一种类型，可选择 1001:市场类 1002:地域类 1003:概念类, 查看[板块分类](</docs2/docs/index.html#板块分类)  
...
参数名 | 类型 | 中文名称 | 必填 | 默认值 | 参数用法说明  
---|---|---|---|---|---  
sector\_code | str | 板块代码 | Y | 无 | 需要查询成分股的板块代码，可通过[stk\_get\_sector\_category](<#stk-get-sector-category-%E6%9F%A5%E8%AF%A2%E6%9D%BF%E5%9D%97%E5%88%86%E7%B1%BB)获取, 查看[板块分类](</docs2/docs/index.html#板块分类)  
...
参数名 | 类型 | 中文名称 | 必填 | 默认值 | 参数用法说明  
---|---|---|---|---|---  
symbol | str | 标的代码 | Y | 无 | 必填，只能填一个股票标的，使用时参考[symbol](变量约定.md#symbol-代码标识)  
...
change\_types | str or list | 异动类型 | N | None | 输入异动类型，可输入多个. 采用 str 格式时，多个异动类型必须用英文逗号分割，如：'106,107'; 采用 list 格式时，多个异动类型示例：\['106','107'\]； 默认None表示所有异动类型。[龙虎榜异动类型列表](</docs2/docs/#龙虎榜异动类型列表)  
...
change\_type | str | 异动类型 | 交易所披露的公开信息及异常波动信息的原因类型,[龙虎榜异动类型列表](</docs2/docs/#龙虎榜异动类型列表)  
