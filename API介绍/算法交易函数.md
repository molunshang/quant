...
类型 | 说明  
---|---  
list\[algoOrder\] | 算法母单委托对象列表，参见[algoOrder 对象](数据结构.md#algoorder-算法委托母单对象)  
...
类型 | 说明  
---|---  
list\[order\] | 委托对象列表，参见[order 对象](数据结构.md#order-委托对象)  
...
algo\_order | [order 对象](数据结构.md#order-委托对象) | 委托  
...
参数 | 类型 | 说明  
---|---|---  
algo\_orders | list\[algoOrder\] | 算法委托对象列表，其中algoOrder为字典，字段参见[algoOrder 对象](数据结构.md#algoorder-算法委托母单对象)  
...
side | int | 参见[订单委托方向](枚举常量.md#orderside委托方向)，OrderSide\_Buy = 1 买入OrderSide\_Sell = 2 卖出  
order\_type | int | 参见[订单委托类型](枚举常量.md#ordertype委托类型)，OrderType\_Limit = 1 限价委托，OrderType\_Market = 2 市价委托  
position\_effect | int | 参见[开平仓类型](枚举常量.md#positioneffect开平仓类型)，PositionEffect\_Open = 1 开仓 PositionEffect\_Close = 2 平仓  
