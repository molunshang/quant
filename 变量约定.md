...
获取订阅的[tick 对象](数据结构.md#tick-tick-对象) 或者 [bar 对象](数据结构.md#bar-bar-对象)滑窗，数据为包含当前时刻推送 tick 或 bar 的前 count 条`tick`或者`bar`数据
...
返回类型为[account - 账户对象](数据结构.md#account-账户对象)。
...
类型 | 说明  
---|---  
list\[position\] | [持仓对象](数据结构.md#position-持仓对象)列表  
...
类型 | 说明  
---|---  
dict\[cash\] | [资金对象](数据结构.md#cash-资金对象)字典  
...
类型 | 说明  
---|---  
dict | key 为动态参数的 key, 值为动态参数对象， 参见[动态参数](API介绍/动态参数.md#动态参数)  
